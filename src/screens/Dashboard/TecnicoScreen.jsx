...
const STATUS_LABEL = {
  generada:   { label: 'Generada',   bg: 'var(--card)',              color: 'var(--muted-foreground)' },
  asignada:   { label: 'Asignada',   bg: 'var(--secco-gold-10)', color: 'var(--secco-gold)' },
  en_proceso: { label: 'En proceso', bg: 'var(--secco-gold)', color: 'var(--secco-gold)' },
  finalizada: { label: 'Finalizada', bg: 'rgba(34,139,80,0.12)', color: 'var(--secco-green)' },
  entregada:  { label: 'Entregada',  bg: 'var(--secco-green)',              color: 'var(--background)' },
}
...
function OTCard({ ot, onClick }) {
  ...
  return (
    <button type="button" onClick={() => onClick(ot)}
      style={{
        width: '100%', textAlign: 'left', background: 'var(--background)',
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
          {ot.numero_ot && (
            <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--secco-gold)', background: 'rgba(169,130,37,0.1)', padding: '2px 7px', borderRadius: 5 }}>
              OT #{ot.numero_ot}
            </span>
          )}
          <span style={{ fontSize: 10, fontWeight: 700, fontFamily: 'monospace', color: 'var(--muted-foreground)', letterSpacing: '1px' }}>
            {otPatente(ot)}
          </span>
        </div>
        <p style={{ margin: '0 0 3px', fontWeight: 600, fontSize: 14, color: 'var(--foreground)' }}>{otVehiculo(ot)}</p>
  ...
  if (loading || !ot) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flex: 1, gap: 12 }}>
        ...
        <span style={{ color: 'var(--muted-foreground)', fontSize: 14 }}>Cargando OT…</span>
      </div>
    )
  }
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
        <button type="button" onClick={onBack}
          style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 8, width: 32, height: 32, fontSize: 16, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--foreground)' }}>←</button>
        <div style={{ flex: 1 }}>
          <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: 'var(--foreground)' }}>
            OT {ot.numero_ot ? `#${ot.numero_ot}` : ''} · {veh.marca} {veh.modelo}
          </p>
          <p style={{ margin: 0, fontSize: 11, color: 'var(--muted-foreground)' }}>{cli.nombre || ''} · {veh.patente || ''}</p>
        </div>
        <span style={{ background: sc.bg, color: sc.color, fontSize: 11, fontWeight: 700, padding: '3px 9px', borderRadius: 6, flexShrink: 0 }}>
          {sc.label}
        </span>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 16px 32px' }}>
        ...
        {/* Datos rápidos */}
        <div style={{ background: 'var(--card)', borderRadius: 10, padding: '12px 16px', marginBottom: 16 }}>
          {[
        ...
        {/* Notas de torre */}
        {ot.notas_torre && (
          <div style={{ background: 'var(--secco-gold-10)', border: '1px solid var(--secco-gold-10)', borderRadius: 10, padding: '12px 16px', marginBottom: 16 }}>
            <p style={{ margin: '0 0 6px', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.8px', color: 'var(--secco-gold)' }}>Instrucciones de torre</p>
            <p style={{ margin: 0, fontSize: 13, color: 'var(--foreground)', lineHeight: 1.6 }}>{ot.notas_torre}</p>
          </div>
        )}

        {/* Repuestos */}
        <div style={{ marginBottom: 16 }}>
          <p style={{ margin: '0 0 10px', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.8px', color: 'var(--muted-foreground)' }}>Repuestos</p>
          {!repuestos.length && <p style={{ margin: 0, fontSize: 13, color: 'var(--muted-foreground)' }}>Sin repuestos asociados.</p>}
          {repuestos.map((rep, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 8, padding: '10px 14px', background: 'var(--background)', border: '1px solid var(--border)', borderRadius: 8 }}>
              <span style={{ fontSize: 11, color: 'var(--placeholder)', minWidth: 18, textAlign: 'right', flexShrink: 0, paddingTop: 2 }}>{i + 1}</span>
              <div style={{ flex: 1 }}>
                <p style={{ margin: 0, fontSize: 13, color: 'var(--foreground)', fontWeight: 500 }}>{rep.nombre || 'Repuesto sin nombre'}</p>
                <p style={{ margin: '2px 0 0', fontSize: 11, color: 'var(--muted-foreground)' }}>Cantidad: {rep.cantidad || 1}</p>
              </div>
              <span style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.3px', color: 'var(--secco-gold)', flexShrink: 0 }}>
                Material
              </span>
            </div>
          ))}
        ...
        {/* Instrucciones */}
        <div style={{ marginBottom: 16 }}>
          <p style={{ margin: '0 0 10px', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.8px', color: 'var(--muted-foreground)' }}>Instrucciones de trabajo</p>
          {!instrucciones.length && <p style={{ margin: 0, fontSize: 13, color: 'var(--muted-foreground)' }}>Sin instrucciones cargadas.</p>}
          {instrucciones.map((ins, i) => {
            const asociados = repuestos.filter((rep) => (ins.repuestos_ids || []).includes(rep.id))
            return (
              <div key={ins.id || i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 10, padding: '12px 14px', background: 'var(--background)', border: '1px solid var(--border)', borderRadius: 8 }}>
                <span style={{ fontSize: 11, color: 'var(--secco-gold)', fontWeight: 700, minWidth: 18, textAlign: 'right', flexShrink: 0, paddingTop: 2 }}>{i + 1}</span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ margin: 0, fontSize: 13, color: 'var(--foreground)', fontWeight: 600, lineHeight: 1.45 }}>{ins.texto}</p>
                  {!!asociados.length && (
                    <div style={{ marginTop: 8, display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                      {asociados.map((rep) => (
                        <span key={rep.id} style={{ fontSize: 11, fontWeight: 600, color: 'var(--secco-gold)', background: 'var(--secco-gold-10)', border: '1px solid rgba(169,130,37,0.24)', borderRadius: 6, padding: '3px 7px' }}>
        ...
        {canAdvance && (
          <button type="button" onClick={handleAvanzar} disabled={avanzando}
            style={{ width: '100%', padding: '14px 0', fontSize: 15, fontWeight: 700, background: 'var(--secco-gold)', color: '#FFF', border: 'none', borderRadius: 12, cursor: 'pointer', fontFamily: 'inherit', marginBottom: 16 }}>
        ...
        {status === 'finalizada' && (
          <div style={{ padding: '14px 16px', background: 'rgba(34,139,80,0.08)', border: '1px solid rgba(34,139,80,0.25)', borderRadius: 12, textAlign: 'center', marginBottom: 16 }}>
            <p style={{ margin: 0, fontSize: 14, fontWeight: 700, color: 'var(--secco-green)' }}>Trabajo finalizado — esperando entrega</p>
          </div>
        )}
        ...
        {historial.length > 0 && (
          <div>
            <p style={{ margin: '0 0 10px', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.8px', color: 'var(--muted-foreground)' }}>Historial</p>
            {[...historial].reverse().map((h, i) => (
              <div key={i} style={{ display: 'flex', gap: 10, marginBottom: 10 }}>
                <div style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--secco-gold)', marginTop: 5, flexShrink: 0 }} />
                <div>
                  <p style={{ margin: '0 0 1px', fontSize: 12, fontWeight: 600, color: 'var(--foreground)' }}>{h.accion}</p>
      ...
      <div style={{ height: '100svh', display: 'flex', flexDirection: 'column', background: 'var(--background)', overflow: 'hidden' }}>
        ...
        <div style={{ borderBottom: '1px solid var(--border)', padding: '0 16px', display: 'flex', alignItems: 'center', gap: 10, height: 52, flexShrink: 0 }}>
          <button type="button" onClick={onVolver}
            style={{ background: 'var(--card)', border: '1px solid var(--border)', color: 'var(--foreground)', borderRadius: 8, width: 32, height: 32, fontSize: 16, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>←</button>
          <img src="/logo-secco.png" alt="SECCO" style={{ height: 24, objectFit: 'contain' }} onError={(e) => { e.target.style.display = 'none' }} />
          <p style={{ margin: 0, fontSize: 14, fontWeight: 600, color: 'var(--foreground)', flex: 1 }}>Vista Técnico</p>
        </div>
    ...
    <div style={{ minHeight: '100svh', background: 'var(--background)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ borderBottom: '1px solid var(--border)', padding: isMobile ? '0 12px' : '0 16px', display: 'flex', alignItems: 'center', gap: 10, height: 52, flexShrink: 0 }}>
        ...
        <button type="button" onClick={cargar} disabled={cargando}
          style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 8, width: 32, height: 32, fontSize: 14, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--muted-foreground)' }}
          title="Actualizar">
      ...
      <div style={{ flex: 1, padding: '16px 16px 40px' }}>
        ...
        {/* Tabs */}
        <div style={{ background: 'var(--card)', borderRadius: 10, padding: 3, display: 'flex', gap: 3, marginBottom: 16 }}>
        ...
        {cargando && (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 40, gap: 12 }}>
            <div style={{ width: 20, height: 20, border: '2px solid #a98225', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} />
            <span style={{ color: 'var(--muted-foreground)', fontSize: 14 }}>Cargando OTs…</span>
          </div>
        )}
