...
function Seccion({ titulo, children }) {
  return (
    <div style={{ marginBottom: 20 }}>
      <p style={{ margin: '0 0 10px', fontSize: 11, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.8px', color: '#1a7a34', borderBottom: '1px solid rgba(26,122,52,0.2)', paddingBottom: 6 }}>{titulo}</p>
...
export default function ActaEntregaDetalleScreen({ actaId, onNavigate, onVolver }) {
  ...
  const fechaFmt = fecha
    ? new Date(fecha + 'T12:00:00').toLocaleDateString('es-CL', { day: '2-digit', month: 'long', year: 'numeric' })
    : '—'
      ...
      <div style={{ background: '#FFFFFF', borderBottom: '1px solid #E0E0E0', padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 10, position: 'sticky', top: 0, zIndex: 40 }}>
        ...
        {esBorrador ? (
          <button
            ...
            style={{ height: 36, padding: '0 12px', borderRadius: 8, border: '1.5px solid #1a7a34', background: '#FFFFFF', color: '#1a7a34', fontSize: 12, fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit' }}
          >
            Editar
          </button>
        ) : null}
        {esAdmin ? (
          <button
            ...
            disabled={eliminando}
            style={{ height: 36, padding: '0 12px', borderRadius: 8, border: '1.5px solid rgba(255,69,58,0.55)', background: 'rgba(255,69,58,0.06)', color: '#FF453A', fontSize: 12, fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit', opacity: eliminando ? 0.6 : 1 }}
          >
            {eliminando ? '…' : 'Eliminar'}
          </button>
        ) : null}
      ...
      <div style={{ padding: '20px 16px' }}>
        ...
        <div className="s-card" style={{ marginBottom: 12 }}>
          <Seccion titulo="Vehículo">
            ...
            {vehiculo.patente ? (
              <div>
                <p style={{ margin: '0 0 2px', fontSize: 10, color: '#6B6B6B', fontWeight: 700, textTransform: 'uppercase' }}>Patente</p>
        ...
        {trabajo ? (
          <div className="s-card" style={{ marginBottom: 12 }}>
            <p style={{ margin: '0 0 10px', fontSize: 11, fontWeight: 800, textTransform: 'uppercase', color: '#1a7a34' }}>Trabajo realizado</p>
